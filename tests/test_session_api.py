...
def test_session_requires_login_401(client):
    resp = client.post(
        "/api/session",
...
def test_session_logged_in_ok(client):
...
def test_session_validation_422(client):
    headers = helpers.admin_headers(client)
    assert client.post("/api/session", json={"section": "不存在", "part": "P1", "count": 2}, headers=headers).status_code == 422
    assert client.post("/api/session", json={"section": SECTION, "part": "P9", "count": 2}, headers=headers).status_code == 422
    assert client.post("/api/session", json={"section": SECTION, "part": "P1", "count": 99}, headers=headers).status_code == 422
